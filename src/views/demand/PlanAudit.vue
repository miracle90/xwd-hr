				...
				>
					...
					<a-row :gutter="24">
						<a-col :span="12" style="font-weight: bold">
							<a-form-item label="需求编号" :label-col="{ span: 6 }">{{
								demandCode
							}}</a-form-item>
						</a-col>
					</a-row>
		...
		distributeSupplierList() {
			if (this.selectSuppliersList.length && this.supplierList.length) {
				...
				return this.selectSuppliersList.map((selectItem, selectKey) => {
					...
					const baseInfo = this.supplierList.filter(
						(item) => item.id === supplierId
					)[0]
					const { supplierName, contactName, contactPhone } = baseInfo
					const {
						demandId,
						dispatchPersions,
						replyPersions,
