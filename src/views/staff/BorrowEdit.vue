					...
					<a-row :gutter="12">
						...
						<a-col :span="12">
							<a-form-item label="已支取" :label-col="{ span: 6 }">
								<a-checkbox
									v-decorator="[`isReceive`, { valuePropName: 'checked' }]"
								></a-checkbox>
						...
						<a-col :span="12">
							<a-form-item label="已代扣" :label-col="{ span: 6 }">
								<a-checkbox
									v-decorator="[`isDeduct`, { valuePropName: 'checked' }]"
								></a-checkbox>
...
export default {
	...
	methods: {
		async getByEmployeeNumber() {
			...
			this.spinning = false
			if (res) {
				const { employeeName, onJobDate, customerName, deptName } = res.data
				...
				if (!error) {
					...
					const param = {
						...
						borrowDate: borrowDate
							? typeof borrowDate === 'string'
								? borrowDate
								: borrowDate.format('YYYY-MM-DD')
							: '',
						receiveDate: receiveDate
							? typeof receiveDate === 'string'
								? receiveDate
								: receiveDate.format('YYYY-MM-DD')
							: '',
						deductYearMonth: deductYearMonth
							? typeof deductYearMonth === 'string'
								? deductYearMonth
					...
					const res = await this.$http.post(
						`/data/borrow/${this.id ? 'update' : 'add'}`,
						param
					)
					this.spinning = false
					if (res) {
						this.$message.success(
							`${this.id ? '预支信息修改成功！' : '新增预支信息成功！'}`
						)
						this.$router.back()
					}
				}
