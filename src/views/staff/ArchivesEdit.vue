              ...
              <a-form-item label="民族" :label-col="{ span: 6 }">
                <a-select
                  ...
                  placeholder="请选择民族"
									show-search
                >
                  <a-select-option :value="item.key" v-for="item in nationList" :key="item.key">{{ item.value }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
						<a-col :span="12">
			...
			this.form.validateFields(async (error, values) => {
				if (!error) {
					...
					const res = await this.$http.post(
						`/data/employee/${this.id ? 'update' : 'add'}`,
						param
					)
					this.spinning = false
					if (res) {
						this.$message.success(
							`${this.id ? '员工档案信息修改成功！' : '新增员工档案成功！'}`
						)
						this.$router.back()
					}
				}
