<template>
  <a-spin class="page-wrapper" :spinning="spinning">
    <a-form :form="form" @submit.prevent="e => {
        page = 1;
        handleSearch(e);
      }" layout="horizontal">
      <a-row :gutter="24">
  ...
  methods: {
        ...
        onOk: async () => {
          this.form.validateFields(async (error, values) => {
            if (!error) {
              ...
              this.spinning = true
              const res = await this.$http.download('/data/rebate/export', '返费设定-导出', param)
              this.spinning = false
              if (res) {
    ...
    downloadTemplet () {
      this.$confirm({
        ...
        onOk: async () => {
          this.spinning = true
          const res = await this.$http.download('/data/rebate/downloadTemplet', '返费设定-模板下载')
          this.spinning = false
          if (res) {
